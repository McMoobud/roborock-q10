...
from .entity import RoborockCoordinatedEntityB01Q7, RoborockCoordinatedEntityB01Q10, RoborockCoordinatedEntityV1
...
Q10_STATE_CODE_TO_STATE = {
    YXDeviceState.UNKNOWN: VacuumActivity.IDLE,
    ...
    YXDeviceState.MAP_SAVE_STATE: VacuumActivity.IDLE,
...
class RoborockVacuum(RoborockCoordinatedEntityV1, StateVacuumEntity):
    ...
    def __init__(self, coordinator: RoborockDataUpdateCoordinator) -> None:
        ...
        RoborockCoordinatedEntityV1.__init__(self, coordinator.duid_slug, coordinator)
    ...
    async def async_set_fan_speed(self, fan_speed: str, **kwargs: Any) -> None:
        ...
        await self.send(
            ...
            [{v: k for k, v in self._device_status.fan_speed_mapping.items()}[fan_speed]],
        )
    ...
    async def async_get_segments(self) -> list[Segment]:
        ...
        if not home_map_info:
            return []
        return [
            Segment(id=f"{map_flag}_{room.segment_id}", name=room.name, group=map_info.name)
    ...
    async def async_clean_segments(self, segment_ids: list[str], **kwargs: Any) -> None:
        ...
        current_map_segments = [seg_id for map_flag, seg_id in parsed if map_flag == current_map]
        if not current_map_segments:
            return
        await self.send(RoborockCommand.APP_SEGMENT_CLEAN, [{"segments": current_map_segments}])
            ...
            "maps": [
                {
                    ...
                    "rooms": {room.segment_id: room.name for room in vacuum_map.rooms},
                }
    ...
    async def get_vacuum_current_position(self) -> ServiceResponse:
        ...
        try:
            ...
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="map_failure") from err
        ...
        if (robot_position := map_content_trait.map_data.vacuum_position) is None:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="position_not_found")
        return {"x": robot_position.x, "y": robot_position.y}


class RoborockQ7Vacuum(RoborockCoordinatedEntityB01Q7, StateVacuumEntity):
    """General Representation of a Roborock Q7 vacuum."""
    ...
    _attr_supported_features = (
        ...
        | VacuumEntityFeature.LOCATE | VacuumEntityFeature.STATE | VacuumEntityFeature.START
    )
    _attr_translation_key = DOMAIN
    _attr_name = None
    ...
    def __init__(self, coordinator: RoborockB01Q7UpdateCoordinator) -> None:
        """Initialize a vacuum."""
        StateVacuumEntity.__init__(self)
        RoborockCoordinatedEntityB01Q7.__init__(self, coordinator.duid_slug, coordinator)
    ...
    async def async_start(self) -> None:
        try:
            ...
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "start_clean"}) from err

    async def async_pause(self) -> None:
        try:
            await self.coordinator.api.pause_clean()
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "pause_clean"}) from err

    async def async_stop(self, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.stop_clean()
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "stop_clean"}) from err

    async def async_return_to_base(self, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.return_to_dock()
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "return_to_dock"}) from err

    async def async_locate(self, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.find_me()
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "find_me"}) from err

    async def async_set_fan_speed(self, fan_speed: str, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.set_fan_speed(SCWindMapping.from_value(fan_speed))
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "set_fan_speed"}) from err

    async def async_send_command(self, command: str, params: dict[str, Any] | list[Any] | None = None, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.send(command, params)
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": command}) from err


class RoborockQ10Vacuum(RoborockCoordinatedEntityB01Q10, StateVacuumEntity):
    """Vacuum entity for Roborock Q10 S5+."""

    _attr_icon = "mdi:robot-vacuum"
    _attr_supported_features = (
        VacuumEntityFeature.PAUSE | VacuumEntityFeature.STOP | VacuumEntityFeature.RETURN_HOME
        | VacuumEntityFeature.FAN_SPEED | VacuumEntityFeature.LOCATE
        | VacuumEntityFeature.STATE | VacuumEntityFeature.START
    )
    _attr_translation_key = DOMAIN
    _attr_name = None
    ...
    def __init__(self, coordinator: RoborockB01Q10UpdateCoordinator) -> None:
        """Initialize a vacuum."""
        StateVacuumEntity.__init__(self)
        RoborockCoordinatedEntityB01Q10.__init__(self, coordinator.duid_slug, coordinator)
    ...
    def activity(self) -> VacuumActivity | None:
        ...
        if data is None or data.status is None:
            return None
        return Q10_STATE_CODE_TO_STATE.get(data.status, VacuumActivity.IDLE)

    @property
    def fan_speed(self) -> str | None:
        data = self.coordinator.data
        if data is None or data.fan_level is None:
            return None
        return data.fan_level.name.lower()

    @property
    def fan_speed_list(self) -> list[str]:
        return [level.name.lower() for level in YXFanLevel if level != YXFanLevel.UNKNOWN]
    ...
    async def async_locate(self, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.command.send("dpSeek", params={})
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "seek"}) from err

    async def async_set_fan_speed(self, fan_speed: str, **kwargs: Any) -> None:
        try:
            await self.coordinator.api.vacuum.set_fan_level(YXFanLevel[fan_speed.upper()])
        except RoborockException as err:
            raise HomeAssistantError(translation_domain=DOMAIN, translation_key="command_failed", translation_placeholders={"command": "set_fan_level"}) from err
        await self.coordinator.async_refresh()
